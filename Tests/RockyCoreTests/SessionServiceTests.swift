...
struct SessionServiceTests {
    ...
    @Test("stop sets endTime on running session for project")
    func stopSession() throws {
        let (projectRepo, _, service) = makeServices()
        let project = try projectRepo.create(name: "acme-corp", slug: "acme-corp".slugified)

        try service.create(projectId: project.id)
        let stopped = try service.stop(projectId: project.id)

        #expect(!stopped.isRunning)
        #expect(stopped.endTime != nil)
    }

    @Test("stop throws when no running session for project")
    func stopNoRunning() throws {
        let (projectRepo, _, service) = makeServices()
        let project = try projectRepo.create(name: "acme-corp", slug: "acme-corp".slugified)

        #expect(throws: RockyCoreError.self) {
            try service.stop(projectId: project.id)
        }
    }

    @Test("stop one project leaves other running")
    func stopOneOfTwo() throws {
        ...
        try service.create(projectId: p1.id)
        try service.create(projectId: p2.id)
        _ = try service.stop(projectId: p1.id)

        let running = try service.list(running: true)
        #expect(running.count == 1)
        #expect(running[0].1.name == "project-2")
    }
    ...
    func stopAll() throws {
        ...
        let stopped = try service.stopAll()
    ...
    func stopAllLeavesCompleted() throws {
        let (projectRepo, sessionRepo, service) = makeServices()
        let p1 = try projectRepo.create(name: "completed", slug: "completed".slugified)
        let p2 = try projectRepo.create(name: "running", slug: "running".slugified)

        _ = try sessionRepo.create(projectId: p1.id, startTime: Date().addingTimeInterval(-7200), endTime: Date().addingTimeInterval(-3600))
    ...
    // MARK: - hasRunningSession

    @Test("hasRunningSession returns false when nothing running")
    func hasRunningFalse() throws {
        let (projectRepo, _, service) = makeServices()
        let project = try projectRepo.create(name: "acme-corp", slug: "acme-corp".slugified)
        #expect(try service.hasRunningSession(projectId: project.id) == false)
    }

    @Test("hasRunningSession returns true after create")
    func hasRunningTrue() throws {
        let (projectRepo, _, service) = makeServices()
        let project = try projectRepo.create(name: "acme-corp", slug: "acme-corp".slugified)
        try service.create(projectId: project.id)
        #expect(try service.hasRunningSession(projectId: project.id) == true)
    }

    @Test("hasRunningSession scoped to project")
    func hasRunningScopedToProject() throws {
        let (projectRepo, _, service) = makeServices()
        let p1 = try projectRepo.create(name: "project-1", slug: "project-1".slugified)
        let p2 = try projectRepo.create(name: "project-2", slug: "project-2".slugified)
        try service.create(projectId: p1.id)
        #expect(try service.hasRunningSession(projectId: p1.id) == true)
        #expect(try service.hasRunningSession(projectId: p2.id) == false)
    }
