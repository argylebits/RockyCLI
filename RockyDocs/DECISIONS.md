...
## Slug-based project lookups

Projects have a `name` (display, stored as provided) and a `slug` (normalized key, used for all lookups and uniqueness). The slug is generated by `String.slugified`: lowercase, non-alphanumeric characters replaced with hyphens, consecutive hyphens collapsed, leading/trailing hyphens trimmed. The database does exact match on slug — no `COLLATE NOCASE`, no case logic in queries. Slugification happens in the service layer, not the repository.

## REST-style command convention

Commands are organized around resources using a `<resource> <action>` pattern, inspired by REST conventions. Each resource is a plural noun with explicit subcommands — no implied default actions.

### Resource groups

```
rocky projects list                     # list all projects
rocky projects rename <old> <new>       # rename a project

rocky sessions start <project>          # start a timer
rocky sessions stop [project] [--all]   # stop a timer
rocky sessions status [flags]           # show time tracking summary
rocky sessions edit [project] [flags]   # edit a session
```

### Rules

- **Resource names are plural nouns** — `projects`, `sessions`
- **No implied subcommands** — `rocky projects` with no subcommand shows help/usage, not a default action
- **Top-level shortcuts** — `rocky start`, `rocky stop`, `rocky status` are hardcoded shortcuts to their `rocky sessions` equivalents for daily workflow convenience. These will be replaced by a user-configurable alias system (#106).
- **`rocky edit` removed as top-level command** — too ambiguous without a resource context. Use `rocky sessions edit`.
- **`rocky projects` replaced** — the old `rocky projects` (which listed projects) becomes `rocky projects list`
- **Standalone commands** — `rocky dashboard` and `rocky config` remain top-level. Dashboard spans multiple resources and doesn't fit under one group. Config is its own resource group (`rocky config get/set/list`).

### Adding new commands

New commands should follow the pattern. Examples of future commands:

```
rocky projects archive <name>
rocky projects show <name>
rocky sessions list [flags]
rocky sessions delete <id>
```

## REST convention extends to all layers

The REST convention applies to the full stack, not just CLI commands:

- **Repository** — pure CRUD (`create`, `get`, `list`, `update`), operates on IDs and slugs. No business logic, no upserts.
- **Service** — business logic, slugifies name inputs, resolves slugs to IDs, validates rules. Get-or-create logic lives at the call site, not hidden inside service methods.
- **Commands** — `<resource> <action>`, top-level shortcuts for daily drivers.
